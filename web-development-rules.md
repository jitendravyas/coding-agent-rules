...
## Browser behaviour, layout, and accessibility
  ...
  * Prefer CSS logical properties and direction-relative values for layouts following content direction, even in single-language projects, subject to browser-support policy. Declare language and direction in HTML; isolate mixed-direction dynamic text where needed. Do not mechanically replace intentional physical directions or assume these defaults prove complete RTL support.
...
## Browser verification
  ...
  * Compare affected visual output when appearance can change. For affected styling and interactions, verify relevant settings such as reduced motion, forced colors, text enlargement, and supported themes. Keep content, controls, and focus indicators perceivable and usable. Responsive layouts or automated checks alone do not prove accessibility; one browser does not prove cross-browser correctness.
