...
## Verification and completion
  ...
  * For added or moved files, reuse verified linter and static-check coverage for matching file types and paths while relevant configuration is unchanged. When coverage is uncertain or relevant configuration changes, inspect selection rules, including custom checks. Fix missing intended coverage in scope or recommend the smallest change; new files alone do not justify new tools or broader checks.
