...
## Implementation and maintenance
  ...
  * For user-facing changes, handle each state affected by the change or needed for the new behaviour. Examples include initial, loading, empty, success, validation, permission, failure, slow-network, offline, retry, and cancellation states. Preserve recoverable input and make the next action clear.
...
## Verification and completion
  ...
  * Prose is not enforcement. Use existing checks and permission controls for mechanical requirements; propose missing controls rather than silently adding hooks, tools, or access. When relying on Git or agent hooks, confirm their trigger, scope, and result.
  ...
  * When relying on a third-party or custom linter or static checker, verify that it selects task-changed files by path and type and applies the intended rules. A passing run may have skipped files or rules. Reuse confirmed coverage while relevant configuration remains unchanged. Respect intentional exclusions. Fix missing intended coverage when in scope; otherwise recommend the smallest change. A coverage gap alone does not justify new tools or unrelated checks.
  * Run relevant project checks, including lint and formatting, scoped where supported. Reuse successful results while relevant code, configuration, dependencies, data, and environment remain unchanged; rerun when evidence may be invalid. Before completion, run required checks and others needed to support the claim; static checks do not prove runtime behaviour. Check representative built output when build, packaging, or runtime configuration matters. Broaden for risk or failures. Run the full suite only when project policy requires it or focused checks cannot establish the result; a commit, PR, or release alone does not trigger it. Include project-required handoff checks; skip unrelated checks and formatting churn.
